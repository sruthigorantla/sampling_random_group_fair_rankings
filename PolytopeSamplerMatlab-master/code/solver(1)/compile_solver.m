...
if (recompile)
   ...
   replaces = replace(replaces, mex_string,'%mex_string');
   replaces = replace(replaces, qdpath,'%qdpath');
   replaces = replace(replaces, libpath,'%libpath');
   replaces = replace(replaces, outputfile,'%outputfile');
   
   for i = 1:length(c)
      c{i} = replace(c{i}, mex_string,'%mex_string');
      c{i} = replace(c{i}, qdpath,'%qdpath');
      c{i} = replace(c{i}, libpath,'%libpath');
      c{i} = replace(c{i}, outputfile,'%outputfile');
